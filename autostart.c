    ...
    for (i = 0; s[i] != '\0'; i++) {
        if (mem_read((uint16_t)(addr + i)) != s[i] % 64) {
            if (mem_read((uint16_t)(addr + i)) != (uint8_t)32) {
                DBG(("check screen buffer failed for: %s, blink mode: %d, mem_read index: %d, mem_read data: %d", s, blink_mode, i, mem_read((uint16_t)(addr + i))));
                return NO;
            }
            return NOT_YET;
        }
...
static void advance_hastape(void)
{
    ...
    switch (check("READY.", AUTOSTART_WAIT_BLINK)) {
        case YES:
            ...
            break;
        case NO:
            DBG(("advance_hastape check result NO"));
            disable_warp_if_was_requested();
            autostart_disable();
            break;
        case NOT_YET:
...
static void advance_pressplayontape(void)
{
    switch (check("PRESS PLAY ON TAPE", AUTOSTART_NOWAIT_BLINK)) {
        case YES:
            ...
            break;
        case NO:
            DBG(("advance_pressplayontape check result NO"));
            disable_warp_if_was_requested();
            autostart_disable();
            break;
        case NOT_YET:
...
static void advance_loadingtape(void)
{
    switch (check("READY.", AUTOSTART_WAIT_BLINK)) {
        case YES:
            ...
            break;
        case NO:
            DBG(("advance_loadingtape check result NO"));
            disable_warp_if_was_requested();
            autostart_disable();
            break;
        case NOT_YET:
...
static void advance_inject(void)
{
    if (autostart_prg_perform_injection(autostart_log) < 0) {
        DBG(("autostart_prg_perform_injection failed"));
        disable_warp_if_was_requested();
        autostart_disable();
    } else {
